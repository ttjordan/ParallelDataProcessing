...
Flights1_filtered = FILTER Flights1 BY (Origin == 'ORD') AND (ArrDelayMinutes is not null) AND (ArrTime is not null) AND (Cancelled is not null) AND (Cancelled neq '1') AND (DepTime is not null) AND (Dest is not null) AND (Diverted is not null) AND (Diverted neq '1') AND (FlightDate is not null) AND (Origin is not null) AND 
((Year == 2007 AND Month >= 6) OR (Year == 2008 AND Month <= 5)) AND ((Year == 2007 AND Month >= 6) OR (Year == 2008 AND Month <= 5));

Flights2_filtered = FILTER Flights2 BY (Dest == 'JFK') AND (ArrDelayMinutes is not null) AND (ArrTime is not null) AND (Cancelled is not null) AND (Cancelled neq '1') AND (DepTime is not null) AND (Dest is not null) AND (Diverted is not null) AND (Diverted neq '1') AND (FlightDate is not null) AND (Origin is not null) AND 
((Year == 2007 AND Month >= 6) OR (Year == 2008 AND Month <= 5)) AND ((Year == 2007 AND Month >= 6) OR (Year == 2008 AND Month <= 5));

join_result = JOIN Flights1_filtered BY (Dest, FlightDate), Flights2_filtered BY (Origin, FlightDate);

filtered_join_result = FILTER join_result by (Flights1_filtered::ArrTime < Flights2_filtered::DepTime);
